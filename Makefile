...
tag=latest
build_base:
	docker build --tag $(project)/base:$(tag)  $$(pwd)/base
build_server:
	docker build --tag $(project)/server:$(tag) $$(pwd)/server
build_data:
	docker build --tag $(project)/data:$(tag) $$(pwd)/data
build_drupal:
	docker build --tag $(project)/drupal:$(tag) .

p_http=8001
p_ssh=9001
ct_name=$(project)
start_mounted:
	docker run --detach --publish $(p_ssh):22 --publish $(p_http):80 \
		--volume $$(pwd)/project:/var/shared/sites/$(project) \
		--name $(ct_name) $(project)/drupal:$(tag)
start:
	docker run --detach --publish $(p_ssh):22 -p $(p_http):80 \
		--name $(ct_name) $(project)/drupal:$(tag)
ssh:
